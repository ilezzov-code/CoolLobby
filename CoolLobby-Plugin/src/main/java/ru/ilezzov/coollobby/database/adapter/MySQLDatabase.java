...
public class MySQLDatabase implements SQLDatabase {
    ...
    public MySQLDatabase(final String host, final int port, final String database, final String username, final String password, final DatabaseType type) {
    ...
    private void createDatabaseIfNotExists() throws SQLException {
        final String baseUrl = url.substring(0, url.indexOf("?")).replaceAll("/[^/]+$", "/");
        final String createUrl = baseUrl + "?useSSL=false&autoReconnect=true";

        try (final Connection conn = DriverManager.getConnection(createUrl, username, password);
             final Statement statement = conn.createStatement()) {

            final String dbName = url.split("/")[3].split("\\?")[0];
    ...
    private void runSchemaFile() throws SQLException, IOException {
        try (final InputStream in = getClass().getClassLoader().getResourceAsStream("data/mysql_schema.sql")) {
            if (in == null) {

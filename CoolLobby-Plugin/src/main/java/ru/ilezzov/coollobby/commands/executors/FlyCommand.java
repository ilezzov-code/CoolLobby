...
public class FlyCommand implements CommandExecutor {
    ...
    private final CooldownManager cooldownManager = new CooldownManager(Main.getConfigFile().getInt("fly_command.cooldown"));
    ...
    public boolean onCommand(@NotNull final CommandSender sender, @NotNull final Command command, @NotNull final String s, final @NotNull String[] args) {
        ...
        api.setFlyMode(player).thenAccept(response -> {
            Bukkit.getScheduler().runTask(Main.getInstance(), () -> {
                switch (response.status()) {
                    case -1 -> sender.sendMessage(PluginMessages.pluginHasErrorMessage(commandPlaceholders));
                    case 0 -> sender.sendMessage(PluginMessages.commandFlyDisableCommandMessage(commandPlaceholders));
                    case 1 -> sender.sendMessage(PluginMessages.commandFlyEnableCommandMessage(commandPlaceholders));
                }
